...
@append @laws
  ...
  docs-are-code: a doc'd change lands with its doc update, same change, on the spot, never deferred; git versions both for the guides; the corpus is untracked working state (@laws#corpus-untracked); nothing waits; close-time verifies, it never updates; a doc-vs-code conflict you notice is real, the doc fix rides the same change
  corpus-untracked: the agent-facing corpus (docs/workspace/) is untracked working state in this repository: gitignored, never committed; the guides (docs/*.md plus README.md) stay tracked and keep the docs-sync law
  corpus-reconcile: code moves reconcile the corpus via the docs-drift flow, never from memory; the audit stays live over the corpus and the check's untracked-claimant verdict (CLEAN (PROCESS-OWNED)) hands the reconciliation to the process
  ...
  docs-audience-separation: documentation surfaces split by audience: user-facing guides (docs/*.md, README.md) own the product depth and low-level physical implementation details (disk layout, file formats, runtime internals) for human maintainers; the agent-facing corpus (docs/workspace/, the docs plugin data) owns the operational contracts, abstract surfaces, and CLI interfaces for agent execution; the agent-facing corpus must remain strictly conceptual and tooling-first, describing surfaces and commands without exposing physical disk paths, filenames, or storage layouts.
