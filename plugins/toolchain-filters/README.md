# toolchain-filters: test runner and compiler filters

The toolchain-filters plugin packages ecosystem-specific output filters for common test runners and compilers.

Contexture's core ships with low-risk filters for Git unified diffs (`diff.awk`), directory listings (`list.awk`), and generic logs (`log.awk`). This plugin adds a menu of modular filters for developer toolchains, without adding external dependencies or maintenance burden to Contexture core.

## Investigate Before Adopting

This plugin is a menu, not a bundle. A filter earns its place only when the workspace actually emits the stream it matches, so inventory first:
...
`ctx run` discovers every filter placed in the builtin run module (`.contexture/modules/run/filters/*.awk`) and in any module's `filters/` directory (e.g. `.contexture/modules/toolchain-filters/filters/*.awk`); the run module scans first, then the other modules by name.
...
Copy the filters the inventory matched into a workspace module, e.g. `.contexture/modules/toolchain-filters/filters/`:

```sh
# One filter:
cp plugins/toolchain-filters/.contexture/modules/toolchain-filters/filters/vitest.awk .contexture/modules/toolchain-filters/filters/

# A matched subset, one line per filter:
cp plugins/toolchain-filters/.contexture/modules/toolchain-filters/filters/vitest.awk .contexture/modules/toolchain-filters/filters/
cp plugins/toolchain-filters/.contexture/modules/toolchain-filters/filters/compiler-errors.awk .contexture/modules/toolchain-filters/filters/

# Ensure executable permissions:
chmod +x .contexture/modules/toolchain-filters/filters/*.awk
```

The filters' test material stays upstream, under `plugins/toolchain-filters/tests/`: the upstream harness pins the plugin's filters, and the adopted copies are byte-identical, so they are pinned too. A workspace carries no test material: the `tests/` folder lives with the plugin, is never copied, and the upstream runner may invoke it.

Do not copy the whole directory by reflex. Each adopted filter joins the discovery scan and can claim a command identity, so the bulk copy is the one adoption path this plugin advises against.
...
It pipes each input through the runner (`ctx run --filter=<filter>`) and byte-compares the output with the expected file. Core cases run through the base runner; plugin cases run through a staged sandbox: the runtime discovers filters in the builtin run module and in workspace modules, so the harness stages a `toolchain-filters` module holding the plugin's filters rather than installing them into the workspace. The presence check fails when a filter carries no fixture pair, so a new filter is not done until it has one. Mechanics cases pin the runner itself: the ANSI strip, command identity in runner mode, the notice-only false-green output, and the recovery fallback to raw.

## Writing Custom Filters

1. Create `.contexture/modules/toolchain-filters/filters/<your-tool>.awk`.
...
7. Add a fixture pair under the matching tests folder (`tests/` for a core filter, `plugins/toolchain-filters/tests/` for a plugin filter) and run the harness.
...
None beyond the base: every filter is a plain awk program, and the runner (`ctx run`) and its guards ship with the convention. No installs and no external tools: the filters receive the command's stream, not its toolchain.

## Contributing

A new filter or an adapted one lands upstream with its fixture pair (the presence check fails without one) and a run of `tests/filter-tests.sh`. Keep each filter's `# match:` and `# command:` headers exact: they are the selection contract. Packaging, naming, and the test convention are in `docs/plugins.md`.
