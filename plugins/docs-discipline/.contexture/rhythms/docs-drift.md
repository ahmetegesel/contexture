@rhythm docs-drift
  ...
  6. VERIFY: ctx docs audit and ctx docs check clean over corpus and the two-sided delta, code plus docs in one stream (the two failure modes live in the ground rule; the working command in the README's two-sided delta paragraph)
