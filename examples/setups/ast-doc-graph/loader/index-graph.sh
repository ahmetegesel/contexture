...
INDEX_GRAPH="$SCRIPT_DIR/index-graph.ts"
...
# 4. Check sibling project directories
if [[ -z "$TSX_BIN" ]]; then
...
# Execute loader: use tsx if found, otherwise use node with native strip-types
if [[ -n "$TSX_BIN" ]]; then
  exec "$TSX_BIN" "$INDEX_GRAPH" "$@"
elif command -v node >/dev/null 2>&1; then
  exec node --experimental-strip-types "$INDEX_GRAPH" "$@"
else
