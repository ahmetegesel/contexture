@rhythm work
  ...
  activation: propose
  1. GATHER: the user's intent, constraints, and pointers, plus a recon lane's facts and open questions; the map first (the index and the owning docs), the repo only for what the docs lacked
  ...
  4. GROUND: ground lanes verify the plan against the docs and the repo; per-task corrections return
  5. AMEND: corrections land in place; material changes return to the human
  6. EXECUTE: one lane per task (small same-shape tasks batch); parallel on independence; the doc change lands with its code change (docs-are-code)
  ...
  8. REVIEW+HARDEN+VERIFY: one fresh lane; independent review, findings hardened, the docs gate re-proven (docs-audit, docs-check, docs-gate)
