...
Alongside the session engine, `.contexture/scripts/compact.sh` provides zero-parameter discoverable stream compaction and command execution. It operates both as a direct runner prefix (`.contexture/scripts/compact.sh <cmd>`) preserving exact exit codes and merging stderr, and as a standard Unix pipe (`cmd | .contexture/scripts/compact.sh`). It auto-discovers and executes modular stream filters placed in `.contexture/filters/*.awk` based on their `# match: <regex>` signature headers. Core Contexture ships low-risk filters for Git unified diffs (`diff.awk`), directory listings (`list.awk`), and generic log deduplication (`log.awk`), with strict fail-safe fallback to raw output if compression fails or produces empty output.
